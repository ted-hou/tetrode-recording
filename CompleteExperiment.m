classdef CompleteExperiment < handle
    ...
    methods
		...
		function [clip, t] = getVideoClip(obj, ephysTime, varargin)
			...
			addParameter(p, 'minLikelihood', 0.95, @isnumeric)
            ...
            minLikelihood       = p.Results.minLikelihood;
        ...
        function vtd = readVideoTrackingData(obj, expName, varargin)
            p = inputParser();
            p.addRequired('expName')
            p.addOptional('side', 'both', @(x) ischar(x) && all(ismember(x, 'flr')))
            p.addOptional('videoExpName', '', @(x) ischar(x) && any(contains(lower(x), {'daisy', 'desmond'})))
            p.addParameter('deeplabcutPath', '')
            p.parse(expName, varargin{:})
            expName = p.Results.expName;
            side = p.Results.side;
            videoExpName = p.Results.videoExpName;
            deeplabcutPath = p.Results.deeplabcutPath;


            if isempty(videoExpName) || videoExpName==""
            ...
            if isempty(deeplabcutPath) || deeplabcutPath==""
                if sidenum ~= 0
                    files_vtd = sortrows(struct2table(dir(sprintf('C:\\SERVER\\%s\\%s\\%s_%g*.csv', animalName, expName, videoExpName, sidenum))), 'datenum', 'descend');
                else
                    files_vtd = sortrows(struct2table(dir(sprintf('C:\\SERVER\\%s\\%s\\%s*.csv', animalName, expName, videoExpName))), 'datenum', 'descend');
                end
            else
                if sidenum ~= 0
                    files_vtd = sortrows(struct2table(dir(sprintf('%s\\%s_%g*.csv', deeplabcutPath, videoExpName, sidenum))), 'datenum', 'descend');
                else
                    files_vtd = sortrows(struct2table(dir(sprintf('%s\\%s*.csv', deeplabcutPath, videoExpName))), 'datenum', 'descend');
                end
            end
...
%                     vtd = addvars(vtd, vel(:, 1), vel(:, 2), spd, 'NewVariableNames', {[splitName{1}, '_VelX'], [splitName{1}, '_VelY'], [splitName{1}, '_Speed']});
    ...
    methods (Static)
        ...
        function ac = readArduino(expName)
            animalName = strsplit(expName, '_');
            datestr = animalName{2};
            animalName = animalName{1};
            file = sprintf('C:\\SERVER\\%s\\%s\\%s.mat', animalName, expName, expName);
            if ~exist(file, 'file')
                file = sprintf('C:\\SERVER\\%s\\%s\\%s_lever_%s.mat', animalName, expName, animalName, datestr);
            end
            if ~exist(file, 'file')
                error('Cannot file arduino file %s', file);
            end
            S = load(file);
            ac = S.obj;
