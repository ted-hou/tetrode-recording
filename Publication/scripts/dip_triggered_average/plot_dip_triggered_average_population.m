...
%% Clear temp vars
clearvars -except xta p kinematics ROOTPATH eu exp expIndices boot eta c
...
load(fullfile(ROOTPATH, "Units\population_dta_rta_25_100_200to800ms_0boots_20260730.mat"));
...
%% Plot dta, rta (Before clustering)
features = ["spikerate", "Jaw", "HandL", "HandR"];
featureDispName = ["spike rate", "jaw pos", "l.hand pos", "r.hand pos"];

xtaMean = struct(dip=[], rise=[]);

for dir = ["dip", "rise"]
    for ifn = 1:length(features)
        fn = features(ifn);
        X = arrayfun(@(xta) xta.(fn).X, xta.(dir), UniformOutput=false);
        X = cat(1, X{:});
        xtaMean.(dir).(fn).X = mean(X, 1, 'omitnan');
        xtaMean.(dir).(fn).t = xta.(dir)(1).(fn).t;
        
        XBoot = arrayfun(@(xta) xta.(fn).XBoot, xta.(dir), UniformOutput=false);
        % XBoot = cat(3, XBoot{:});
        % xtaMean.(dir).(fn).XBoot = mean(XBoot, 3, 'omitnan');
        xtaMean.(dir).(fn).XBoot = cat(1, XBoot{:});
    end
end


fig = figure(Units='inches', Position=[1, 1, 5, 1.5]);
tl = tiledlayout(fig, 1, length(features), TileSpacing='tight', Padding='tight');
iRow = 0;
alpha = 0.05;
% for dir = ["dip", "rise"]
for dir = "dip"
    ...
    for fn = features
        ...
        t = xtaMean.(dir).(fn).t;
        mu = xtaMean.(dir).(fn).X;
        hi = quantile(xtaMean.(dir).(fn).XBoot, 1-alpha/2, 1);
        lo = quantile(xtaMean.(dir).(fn).XBoot, alpha/2, 1);

        plot(ax, t, mu, Color=[0.15, 0.15, 0.15], LineWidth=1.5, DisplayName=fn)
        patch(ax, [t, flip(t)], [hi, flip(lo)], [0.15, 0.15, 0.15], FaceAlpha=0.1, EdgeAlpha=0.1)
        ylim(ax, [-1, 1])

        % std boot stats (significance)
        % probability of observing this much movement or more by chance
        % if fn ~= "spikerate"
        %     stdObs = mean(arrayfun(@(xta) xta.(fn).stats.std, xta.(dir)), 'omitnan');
        %     stdBoot = arrayfun(@(xta) xta.(fn).stats.stdBoot, xta.(dir), UniformOutput=false);
        %     stdBoot = mean(cat(2, stdBoot{:}), 2, 'omitnan');
        %     pValCombined = nnz(stdBoot>=stdObs)./length(stdBoot);
        %     pVal = arrayfun(@(xta) nnz(xta.(fn).stats.stdBoot>=xta.(fn).stats.std)./length(xta.(fn).stats.stdBoot), xta.(dir));
        %     title(ax, sprintf("%s - %s, p_{all}=%.4f, %i/%i (%i%%) sig", featureDispName(iCol), dir, pValCombined, nnz(pVal<alpha), length(pVal), round(100*nnz(pVal<alpha)./length(pVal))))
        % else
        %     title(ax, sprintf("%s - %s", featureDispName(iCol), dir))
        % end
        title(ax, sprintf("%s", featureDispName(iCol)))
        if iCol == 1
            ylabel(ax, 'z-score')
        end
    end
...
xlabel(tl, 'time from dip onset (s)')
fontsize(fig, 8, 'points')

nDips = arrayfun(@(xta) length(xta.t0), xta.dip);
nRises = arrayfun(@(xta) length(xta.t0), xta.rise);
...
%         p.mi.clusterSeed = 47; % [-0.3, -0] vs [0, 0.3]: 43,47,42(hands are correlated a bit); [-0.6, -0] vs [0, 0.6]: 42; [-1, -0.3] vs [0, 0.6]: 42, 2
%         % p.mi.semanticClusterOrder = [1, 3, 2, 6, 7, 5, 4];
%         p.mi.semanticClusterOrder = 1:6;
...
% 
%         v = matlabRelease();
%         if v.Date >= datetime(2026, 4, 20) % umap was added in 2026a
...
%         for dir = ["dip", "rise"]
%             n = nTrials.(dir)(iExp);
%             mi.(dir)(iExp).idx = idx.(dir)(i0.(dir)+1 : i0.(dir)+n);
%             mi.(dir)(iExp).pcaScore = pcaScore.(dir)(i0.(dir)+1 : i0.(dir)+n, :);
%             mi.(dir)(iExp).dispScore = dispScore.(dir)(i0.(dir)+1 : i0.(dir)+n, :);
%             i0.(dir) = i0.(dir) + n;
%         end
%     end
...
%         for dir = ["dip", "rise"]
%             idx = mi.(dir)(iExp).idx;
%             clusterSize(iExp).(dir).n = zeros(1, p.mi.nClusters);
%             clusterSize(iExp).(dir).nRaw = zeros(1, p.mi.nClusters);
%             clusterSize(iExp).(dir).nTotal = length(idx);
%             clusterSize(iExp).(dir).threshold = p.mi.minNumTrialsPerCluster;
%             for k = 1:p.mi.nClusters
%                 n = nnz(idx==k);
%                 clusterSize(iExp).(dir).nRaw(k) = n;
%                 if n < clusterSize(iExp).(dir).threshold
%                     clusterSize(iExp).(dir).n(k) = NaN;
%                 else
%                     clusterSize(iExp).(dir).n(k) = n;
%                 end
%             end
%         end
%     end
%     clear iExp dir idx k n
% else
...
%     for dir = ["dip", "rise"]
%         for k = 1:p.mi.nClusters
%             sel = mi.(dir)(iExp).idx==k;
%             for fn = features
%                 mp.(dir)(iExp, k).(fn) = xta.(dir)(iExp).(fn).X(sel, :);
%             end
%         end
%     end
% end
% clear mp0 iExp dir k sel fn
% 
...
%                         nTrials = clusterSize(iExp).(dir).n(k);
...
%                             else
%                                 hVal = cc.data(iExp).(dir).h(iClu, iFeat-1);
%                                 nStars = nnz(hVal);
...
%                     title(tl, sprintf("Unit %i (n=%i %ss)", iExp, length(xta.(dir)(iExp).t0), dir), FontWeight='bold')
%                     fontsize(fig, 8, 'points')
%                     print(fig, fullfile(exportPath, sprintf("featxclus_unit_%03i_%s", iExp, dir)), '-dpng', '-r0')
%                 end
%             end
%         end
%     end
% end
% 
% clear features featureDispName featureUnits featureSign featureAxisDir semanticClusterSign clusterDispOrder ylims xt nClusters nFeatures fig tl ax iClu iFeat iTest nCleanGroups exportPath selUnits iExp dir idx iClu k nTrials fn sel c X t mu xObs xBoot pVal nStars 
