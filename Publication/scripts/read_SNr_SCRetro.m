...
parser.addParameter('recalculateBootstrap', true, @islogical)
parser.addParameter('recalculateETA', true, @islogical)
...
parser.addParameter('groupRedPowersAbove', 8000*1e-6, @isnumeric)
...
recalculateBootstrap = parser.Results.recalculateBootstrap;
recalculateETA = parser.Results.recalculateETA;
...
groupRedPowersAbove = parser.Results.groupRedPowersAbove;
groupRedPowersAboveName = sprintf("%i+", round(groupRedPowersAbove*1e6));
...
if ~isempty(metaPath) % 'C:\SERVER\Units\meta_TwoColor_SNr_SCRetro.mat'
    load(metaPath)
    if ~recalculateBootstrap && ~recalculateETA
        return
    end
end
...
X = X(1:length(redPowers) + 2);
XRed = dictionary([string(redPowers*1e6), groupRedPowersAboveName, "all"]', X);
...
for iEu = 1:length(eu)
    ...
    for iGrp = 1:length(groupsRed) + 1
        if iGrp <= length(groupsRed)
            ...
            if groupsRed(iGrp).power < groupRedPowersAbove
                key = string(groupsRed(iGrp).power*1e6);
            else
                key = groupRedPowersAboveName;
            end
        else
...
% Boot response dir
if isempty(metaPath) || recalculateBootstrap
    p.bootAlpha = 0.05;
    p.nboot = 100000;
    p.metaWindow = [-0.3, 0];
