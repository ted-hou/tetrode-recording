...
EphysUnit.plotETA(axTemp, eta.press, c.hasPress, xlim=[-4,0], clim=[-1.5, 1.5], ...
...
EphysUnit.plotETA(axTemp, eta.lick, c.hasLick, xlim=[-4,0], clim=[-1.5, 1.5], ...
...
latency.pRankSum.pressVsContraPaw = ranksum(tst, latency.press(c.isPressResponsive), tail='right');
latency.pRankSum.pressUpVsContraPaw = ranksum(tst, latency.press(c.isPressUp), tail='right');
latency.pRankSum.pressDownVsContraPaw = ranksum(tst, latency.press(c.isPressDown), tail='right');
latency.pRankSum.pressVsLick = ranksum(latency.lick, latency.press, tail='right');
latency.pRankSum.pressVsContraPawPlus200 = ranksum(tst-0.2, latency.press(c.isPressResponsive), tail='right');
...
% 4a PETH of increase vs. decrease populations with arrows pointing to mean
% onset time
ax = gobjects(3, 1);
...
hold(ax(2), 'on')
hHist = gobjects(2, 1);
hHist(1) = histogram(ax(2), latency.press(c.isPressUp), edges, Normalization='count', DisplayName=LABEL(1), FaceColor=COLOR(1), FaceAlpha=0.6);
hHist(2) = histogram(ax(2), latency.press(c.isPressDown), edges, Normalization='count', DisplayName=LABEL(2), FaceColor=COLOR(2), FaceAlpha=0.6);
...
hLgd = legend(sprintf('%g trials\n%g sessions', nnz(~isnan(tst)), 7), Location='northwest');
hLgd.Position(1) = 0.24;
hLgd.Position(2) = 0.195;
