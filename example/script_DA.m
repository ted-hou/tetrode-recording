...
%% Plot BinnedByTrialLength ETAs (population average)
[bta.X, bta.T, bta.N, bta.S, bta.B] = eu.getBinnedTrialAverage('rate', edges=2:2:10, resolution=0.001);
EphysUnit.plotBinnedTrialAverage(axes(figure), bta)

%% Plot BinnedByTrialLength ETAs (single units)
close all
for iEu = 1:length(eu)
    clear bta
    [bta.X, bta.T, bta.N, bta.S, bta.B] = eu(iEu).getBinnedTrialAverage('rate', edges=2:2:10, resolution=0.001);
    EphysUnit.plotBinnedTrialAverage(axes(figure), bta, showTrialNum=true);
end
 
%% Plot Binned ETAs (population average) aligned to CUE
[bta.X, bta.T, bta.N, bta.S, bta.B] = eu.getBinnedTrialAverage('rate', trialType='lick', edges=2:2:10, resolution=0.001, alignTo='start', window=[0, 10]);
EphysUnit.plotBinnedTrialAverage(axes(figure), bta)

%% Plot Binned ETAs (single units) aligned to CUE
close all
for iEu = 1:length(eu)
    clear bta
    [bta.X, bta.T, bta.N, bta.S, bta.B] = eu(iEu).getBinnedTrialAverage('rate', edges=2:2:10, resolution=0.001, alignTo='start', window=[0, 10]);
    EphysUnit.plotBinnedTrialAverage(axes(figure), bta, showTrialNum=true);
end
