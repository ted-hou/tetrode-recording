...
for iEu = 1:length(eu)
    ...
    groupsTemp = eu(iEu).groupTwoColorStimTrials({'wavelength', 'power', 'duration', 'location'}, selectBy=struct(power=[], duration=10e-3, location=[], wavelength=[]));
    for iCondition = 1:length(groupsTemp)
         etaTemp = eu(iEu).getETA('count', 'stim', window=p.etaWindow, resolution=p.etaRes, trials=groupsTemp(iCondition).trials, alignTo='start', normalize='none');...p.etaBaselineWindow);
         etaTemp.X = (etaTemp.X - mean(etaTemp.X(isin(etaTemp.t, p.etaBaselineWindow))))./p.etaRes; % Substract baseline spike rate and divide by binWidth (i.e., change in spikes/s relative to baseline)
         eta{iEu, iCondition} = etaTemp;
    end
    groups{iEu} = groupsTemp;
...
clear iEu linelength groupsTemp iCondition etaTemp

% You should end up with two variables: `eta` and `groups`
% `groups` (nUnits x 1) contains a list of all stimulation conditions (they should be the same across all units, assuming we used the same set of stimulation conditions across all recording sessions).
% `eta`  (nUnits x nConditions) contains a PSTH (t, X) for each stim condition for each unit. `groups{iUnit}(iCondition)` should give you a description of the corresponding condition.
%       eta{iUnit, iCondition}.t: timestamps relative to opto pulse onset
%       eta{iUnit, iCondition}.X: change in spike rate (sp/s) relative to baseline (-0.5s, -0.1s) before opto onset
