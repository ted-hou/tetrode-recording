...
clear, clc
tr = TetrodeRecording();
tr.SelectFiles(NeuropixelPath='C:\SERVER\desmond42\desmond42_20251030')
tr.LoadNeuropixelIO();
tr.ParseNeuropixelIO(DigitalChannels={'Sync', 0; 'Lick', 1; 'Press', 2; 'Reward', 3; 'Timeout', 4; 'Mot2Busy', 5; 'CueLeft', 6; 'CueRight', 7});
...
p.xlim.stim = [-0.1, 0.3];
p.xlim.move = [-4, 2];
p.path = 'C:\SERVER\Figures\TwoColor_Striatonigral\TestBatch';
p.rasterSzStim = 1;
p.rasterSzMove = 1;
...
selUnitsStim = c.isStimBlueUp | c.isStimRedUp | c.isStimBlueDown | c.isStimRedDown;
close all
fig = figure(Units='inches', Position=[1, 1, 4.5, 5]);
ETASORT = {eta.stimBlue, eta.stimRed};
SORTWINDOW = {[0, 50]*1e-3, [0, 50]*1e-3};
...
NAMECOLOR = ["blue", "red"];
ZEROLABEL = ["stim", "stim"];
XLIM = {[-25, 50]*1e-3, [-25, 50]*1e-3};
XTICKS = {[0, 20, 50]*1e-3, [0, 20, 50]*1e-3};
CLIM = {[-10, 10], [-10, 10]};
CBTILE = {'', 'east'};
