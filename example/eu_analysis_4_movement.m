...
ax = gobjects(5, 2);
for i = 1:5
    ax(i, 1) = subplot(5, 2, 2*(i-1)+1);
    plotETAComparison(ax(i, 1), eta.press, eta.lick, SEL{i}, 'press', 'lick', yUnit='a.u.');
    ax(i, 2) = subplot(5, 2, 2*(i-1)+2);
    plotETAComparison(ax(i, 2), eta.press, eta.lick, SEL{i} & c.isLick, 'press', 'lick', yUnit='a.u.');
end
xlabel()
clear ax fig i
...
clear boot
[boot.h, boot.muDiffCI, boot.muDiffObs] = bootstrapMoveResponse(eu, 'press', alpha=0.01, withReplacement=false);
figure, histogram(boot.h)
...
function [h, muDiffCI, muDiffObs] = bootstrapMoveResponse(eu, trialType, varargin)
    ...
    p.addParameter('allowedTrialDuration', [2, 4], @(x) isnumeric(x) && length(x) >= 2 && x(2) >= x(1))
...
function [ax, fig] = plotETAComparison(varargin) % NonHeatmap
    ...
    if r.averageAcrossUnits
        ...
        plot(ax, t1, x1, 'r', DisplayName=label1)
        plot(ax, t2, x2, 'b', DisplayName=label2)
        hold(ax, 'off')
        if ~isempty(r.xlim)
            xlim(ax, r.xlim)
        end
