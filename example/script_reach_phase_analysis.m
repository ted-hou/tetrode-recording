...
expName = unique(string({eu(sel).ExpName})');
animalName = arrayfun(@(expName) strsplit(expName, "_"), expName, UniformOutput=false);
animalName = lower(string(cellfun(@(n) n{1}, animalName, UniformOutput=false)));
nSessionsInAnimal = arrayfun(@(n) nnz(animalName==n), animalName);
nUnitsInAnimal = arrayfun(@(animalName) nnz(ismember(string(eu(sel).getAnimalName())', animalName)), animalName);
nUnitsInSession = arrayfun(@(expName) nnz(ismember(string({eu(sel).ExpName})', expName)), expName);

reachVsLick = table(animalName, expName, nSessionsInAnimal, nUnitsInAnimal, nUnitsInSession);
clear sel expName animalName nSessionsInAnimal nUnitsInSession nUnitsInAnimal

% reachVsLick = sortrows(reachVsLick,"nUnits","descend");

%%
animals.oneCam = ["daisy2", "daisy3", "daisy8", "daisy9"];
animals.twoCam = ["daisy14", "daisy15", "daisy16", "desmond23", "desmond24", "desmond25", "desmond26", "desmond27"];

%% Align vtd data
selSession = ismember(reachVsLick.animalName, animals.twoCam);
selEu = ismember(string({eu.ExpName}), reachVsLick.expName(selSession));

exp = CompleteExperiment3(eu(selEu), cameras='lr');
exp.alignTimestamps(refEventNameArduino='CUE_ON', refEventNameEphys='Cue', trialDurationTolerance=2, shiftDurationTolerance=2);

%% Select good exps with vtd data
expGood = exp(arrayfun(@(exp) size(exp.vtdR, 2)>2, exp));
expAll = exp;
exp = expGood;
...
%% Get reachStart, reachEnd, retractStart, retractEnd