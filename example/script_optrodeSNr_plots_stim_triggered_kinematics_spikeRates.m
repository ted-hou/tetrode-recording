            ...
            for iDuration = 1:2
                ...
                switch fn
                    ...
                    case 'move'
                        ...
                        N(~isfinite(N)) = 0;
                        h(iLine) = histogram(ax, BinEdges=edges, BinCounts=N, DisplayStyle='bar', EdgeColor='none', FaceColor=colorsByPower(iPower+1, 1:3), FaceAlpha=0.5, DisplayName=sprintf("ctrl (n=%i)", nnz(selCtrl)));
                        iLine = iLine + 1;
                    case {'X', 'XInc', 'XDec', 'XFlt'}
                        ...
                        h(iLine) = plot(ax, t, mumu, Color=colorsByPower(iPower+1, :), LineWidth=1.5, DisplayName=sprintf("ctrl (n=%i)", nnz(selCtrl)));
                        iLine = iLine + 1;
                end
                % Stim traces
                ...
                for iHash = 1:length(uniqueHash)
                    ...
                    switch fn
                        case {'Jaw', 'HandL', 'HandR', 'JawInhibited', 'HandLInhibited', 'HandRInhibited', 'JawBreakthrough', 'HandLBreakthrough', 'HandRBreakthrough'}
                            ...
                            h(iLine) = plot(ax, tLocal, mu, Color=colorsByPower(iPower+1, :), LineStyle=lineStylesByPower(iPower+1), LineWidth=1.5, DisplayName=sprintf("%s (n=%i)", label, nnz(sel)));
                            iLine = iLine + 1;
                        ...
                        case {'move'}
                            ...
                            N(~isfinite(N)) = 0;
                            h(iLine) = histogram(ax, BinEdges=edges, BinCounts=N, DisplayStyle='stairs', EdgeColor=colorsByPower(iPower+1, 1:3), EdgeAlpha=colorsByPower(iPower+1, 4), LineWidth=1.5, DisplayName=sprintf("%s (n=%i)", label, nnz(sel)));
                        case {'X', 'XInc', 'XDec', 'XFlt'} % Spike rate
                            ...
                            h(iLine) = plot(ax, t, mumu, Color=colorsByPower(iPower+1, :), LineWidth=1.5, DisplayName=sprintf("%s (n=%i)", label, nnz(sel)));
                            iLine = iLine + 1;
                            % yline(ax, 0, 'k--')
                    end
                end
