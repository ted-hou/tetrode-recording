...
%
% mkdir('Z:\Units\acute_3cam_reach_direction\Pawnalyzer2')
% pa.save('Z:\Units\acute_3cam_reach_direction\Pawnalyzer2\pa.mat')
%
pa.load('Z:\Units\acute_3cam_reach_direction\Pawnalyzer2\pa.mat')
...
fig1 = figure();
ax1 = axes(fig1);
fig2 = figure(Units='normalized', OuterPosition=[0.1, 0.1, 1.9, 0.8]);
ax2 = gobjects(nExp, 1);
ax3 = gobjects(nExp, 1);
hold(ax1, 'on')
clear traj
...
for iExp = 1:nExp
    ...
    ax3(iExp) = subplot(2, nExp, iExp + nExp);
    xlabel(ax3(iExp), 'X')
    ylabel(ax3(iExp), 'Y')
    zlabel(ax3(iExp), 'Z')
    hold(ax3(iExp), 'on')
    ...
    for iTarget = 1:4
        for iSubCluster = 1:nSubClusters
            ...
            % assert(all(y(:, 1) == 0))
            % assert(all(z(:, 1) == 0))
            plot3(ax3(iExp), median(x, 1, 'omitnan'), median(y, 1, 'omitnan'), median(z, 1, 'omitnan'), Color=colors(iTarget), DisplayName=targetNames{iTarget}, LineWidth=1.5);
